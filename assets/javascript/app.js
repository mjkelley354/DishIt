...
if ($("body").attr("data-title") === "index-page") {
    // functions run on load of index-page
...
function getTopX(recordsToReturn) {
    ...
    dishes.orderByChild("avgRating")
        .limitToLast(recordsToReturn)
        .on("child_added", function(snapshot) {
        const keyValue = snapshot.ref.key;

        dishes.child("/" + keyValue).once("value", function(dishesSnapshot) {
            restaurants
            .child("/" + dishesSnapshot.val().restaurantId)
            .once("value", function(restaurantSnapshot) {
                newDish(
                keyValue,
  ...
  $("tbody").prepend(
    `
            <tr class="dish-tile" id="heading${i}" dish-id-value="${dishId}" data-toggle="collapse" data-target="#collapse${i}">
                ...
                <td class="align-middle rating-icons p-0">${getRating(
                  avgRating
                )}<br><span style="color:black;">${dishPrice}</span>
                </td>
            </tr>
            <tr>
...
function createTiles(dishArray) {
  ...
  for (let i in dishArray) {
    ...
    $("tbody").prepend(
      `
                <tr class="dish-tile" id="heading${i}" dish-id-value="${
        dishArray[i].dishId
      }" data-toggle="collapse" data-target="#collapse${i}">
                    <td><img class="dish-tile-img" src="assets/images"></td>
                    <td class="align-middle"><h6>${dishArray[i].dishName}<br>@${
        dishArray[i].restaurantName
      }</h6></td>
                    <td class="align-middle">${getRating(
                      dishArray[i].avgRating
                    )}<br>${getPrice(dishArray[i].dishPrice)}</td>
                </tr>
                <tr>
                    <td colspan=5 class="collapse" id="collapse${i}">placeholder</td>
                </tr>
...
function getSalty(avgSaltyScale) {
    ...
    if (Math.round(avgSaltyScale * 2) / 2 - salt === 0.5) {
...
function getSour(avgSourScale) {
    ...
    if (Math.round(avgSourScale * 2) / 2 - sour === 0.5) {
...
function getSpicy(avgSpicyScale) {
  ...
  if (Math.round(avgSpicyScale * 2) / 2 - spicy === 0.5) {
...
function getSweet(avgSweetScale) {
  ...
  if (Math.round(avgSweetScale * 2) / 2 - sweet === 0.5) {
...
function getUmami(avgUmamiScale) {
  ...
  if (Math.round(avgUmamiScale * 2) / 2 - umami === 0.5) {
...
$("#search-btn").on("click", function() {
  ...
  $(".apply-filter").on("click", function() {
    ...
    for (let i = 0; i < dishArray.length; i++) {
      ...
      if (!isInRange(dishArray[i].avgSourScale, sourMin, sourMax)) {
        console.log(
          "Sour scale is out of range! min: " +
            ratingMin +
            " max: " +
            priceMax +
            " actual: " +
            dishArray[i].avgSourScale
        );
        $("tr[dish-id-value='" + dishArray[i].dishId + "']").hide();
        continue;
      } else {
      ...
      if (!isInRange(dishArray[i].avgSweetScale, sweetMin, sweetMax)) {
        console.log(
          "Sweet scale is out of range! min: " +
            ratingMin +
            " max: " +
            priceMax +
            " actual: " +
            dishArray[i].avgSweetScale
        );
        $("tr[dish-id-value='" + dishArray[i].dishId + "']").hide();
        continue;
      } else {
      ...
      if (!isInRange(dishArray[i].avgSpicyScale, spicyMin, spicyMax)) {
        console.log(
          "Spicy scale is out of range! min: " +
            ratingMin +
            " max: " +
            priceMax +
            " actual: " +
            dishArray[i].avgSpicyScale
        );
        $("tr[dish-id-value='" + dishArray[i].dishId + "']").hide();
        continue;
      } else {
      ...
      if (!isInRange(dishArray[i].avgSaltyScale, saltyMin, saltyMax)) {
        console.log(
          "Salty scale is out of range! min: " +
            ratingMin +
            " max: " +
            priceMax +
            " actual: " +
            dishArray[i].avgSaltyScale
        );
        $("tr[dish-id-value='" + dishArray[i].dishId + "']").hide();
        continue;
      } else {
      ...
      if (!isInRange(dishArray[i].avgUmamiScale, umamiMin, umamiMax)) {
        console.log(
          "Umami scale is out of range! min: " +
            ratingMin +
            " max: " +
            priceMax +
            " actual: " +
            dishArray[i].avgUmamiScale
        );
        $("tr[dish-id-value='" + dishArray[i].dishId + "']").hide();
        continue;
      } else {
  ...
  $(".map-modal").on("shown.bs.modal", function() {
    ...
    myLatlng = new google.maps.LatLng(33.753746, -84.38633);
  ...
  function setValues(stepIncrease) {
    return function(event, ui) {
      ...
      if (
        !(
          Math.abs(ui.values[0] - currentValues[0]) == stepIncrease * step ||
          Math.abs(ui.values[1] - currentValues[1]) == stepIncrease * step
        )
      ) {
        return false;
      }
      ...
      var currentValues = slider.slider("values");
      $("#" + this.id + "-values").html(
        currentValues[0] + " to " + currentValues[1]
      );
    };
  }
  $(".slider-1-5").slider({
  ...
  function createTestData() {
    ...
    let taqueriaDelSolId = writeRestaurantData(
      "123",
      "Taqueria del Sol",
      "433 N. McDonough St.",
      "Atlanta",
    ...
    let sapporoId = writeRestaurantData(
      "456",
      "Sapporo de Napoli",
      "",
      "Atlanta",
    ...
    let grindhouseId = writeRestaurantData(
      "789",
    ...
    let tacoId = writeDishData(
      ...
      2.76,
      5,
      "https://firebasestorage.googleapis.com/v0/b/dish-it.appspot.com/o/images%2Fbeef-tacos.jpg?alt=media&token=c0f7b553-373f-4f0d-bea7-22cd524c1fe5",
      1
    );
    console.log("tacoId: ", tacoId);
    let cheesePizzaId = writeDishData(
      "cheese pizza",
      sapporoId,
      "",
      2,
      ...
      2,
      3.6,
      "https://firebasestorage.googleapis.com/v0/b/dish-it.appspot.com/o/images%2Fcheese-pizza.jpg?alt=media&token=ced316ad-ab07-4146-b6ea-04f7e400980b",
      1
    );
    console.log("cheesePizzaId: ", cheesePizzaId);
    let cheeseburgerId = writeDishData(
      "cheeseburger",
      grindhouseId,
      "",
      2,
      ...
      1,
      4.24,
      "https://firebasestorage.googleapis.com/v0/b/dish-it.appspot.com/o/images%2Fcheeseburger.jpg?alt=media&token=bbbf89f2-1d7a-4246-aed5-0f3b51883302",
      1
    );
    console.log("cheeseburgerId: ", cheeseburgerId);

    let tacoRating = writeRatingData(
      tacoId,
      "beef taco supreme",
      "123",
      larryId,
    ...
    let pizzaRating = writeRatingData(
      cheesePizzaId,
      "cheese pizza",
      "456",
      moeId,
      "Best Pizza in Decatur!!",
      1,
      2,
      3,
      1,
    ...
    let burgerRating = writeRatingData(
      cheeseburgerId,
      "cheeseburger",
      "789",
      curlyId,
      "Burgers are better than FarmBurger and shakes too!",
      1,
      2,
      2,
      2,
...
$("#find-restaurant").on("click", function() {
        const location = $("#city-input").val()
            .trim() +
        ", " +
        $("#state-input")
            .val()
            .trim();
        rNameInput = $("#restaurant-input")
        .val()
        .trim();
...
/*   }); */
  ...
  $.ajax({
    ...
    headers: {
      Authorization:
        "Bearer lC3zgwezYWCKbJZW03Yepl4A52o_fhrqd9a1x0_MapVxItu97aAHOUOGfsRzDJswOWzWlaHv0zvw8keaePumFEkXJWyOgcTcLg7ekQOQ9skybUd_wy02lE3hnQy0W3Yx"
    }
  ...
  $("#restaurant-results").append(
    `
            <div class="form-check">
                ...
                value="${restaurant.id}" index="${i}" r-name="${
      restaurant.name
    }" 
                address1="${restaurant.location.address1}"
                address2="${restaurant.location.city}, ${
...
function addRestaurant() {
  ...
  rPrice = getPriceAsNumber(yelpDataObject.price);
  console.log(rPrice);
  ...
  restaurantRecords
    ...
    .once("value", function(snap) {
      if (snap.val() === null) {
        rIdFirebase = writeRestaurantData(
          yelpId,
          rName,
          address,
          city,
...
function addDish() {
    ...
    let matches = 0;
    dishRecords.orderByChild("name")
        .equalTo(dName) .once("value", function(dishSnapshot) {
        console.log(dishSnapshot.val());
        console.log(downloadURL);
        if (dishSnapshot.val() === null) {
            dishIdFirebase = writeDishData(
            dName,
            rIdFirebase,
            rPrice,
            0,
